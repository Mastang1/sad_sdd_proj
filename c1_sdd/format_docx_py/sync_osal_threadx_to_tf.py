...
3. 在 ``ipcsOsPollChannels`` 流程图之后插入 **3.6.35 / 3.6.36** 结构体小标题与 Markdown 三列表格（与 4.5 FREERTOS 节末尾结构体版式一致）。
4. 更新 THREADX 小节标题，去掉 ``（todo）`` 字样。
...
def parse_md_pipe_table(table_md: str) -> tuple[int, int, list[tuple[int, int, int, int, str]]]:
    """将 Markdown 管道表解析为 Word 表格 placements（无合并）。"""
    ...
    for r, row in enumerate(rows):
        for c in range(ncols):
            ...
            placements.append((r, c, 1, 1, text))
    return nrows, ncols, placements


def load_threadx_struct_blocks(md_path: Path) -> list[tuple[str, str]]:
    text = md_path.read_text(encoding="utf-8")
    start = text.find("## OSAL(THREADX)")
    end = text.find("## Dynamic Detailed Design", start)
    chunk = text[start:end]
    found = _RE_STRUCT_BLOCK.findall(chunk)
    if len(found) != 2:
        raise SystemExit(
            f"THREADX 小节期望 2 个结构体表，实际 {len(found)}：{[f[0] for f in found]}"
        )
    return [(title, table_md) for title, table_md in found]
...
def sync_section(
    doc: Document,
    blocks: list[tuple[str, str, Path]],
    struct_blocks: list[tuple[str, str]],
) -> None:
    ...
    for title, table_md in struct_blocks:
        anchor = insert_paragraph_after(doc, anchor, title, style="Heading 3")
        nrows, ncols, placements = parse_md_pipe_table(table_md)
        if nrows == 0 or ncols == 0:
            raise SystemExit(f"{title} 结构体表格解析失败")
        anchor = insert_table_after(doc, anchor, nrows, ncols, placements)
    print(f"  删除旧块元素: {n_rm}")
    print(f"  插入函数: {len(blocks)}")
    print(f"  插入结构体: {len(struct_blocks)}")
...
def main() -> None:
    ...
    struct_blocks = load_threadx_struct_blocks(md)
    doc = Document(str(tf))
    sync_section(doc, blocks, struct_blocks)
