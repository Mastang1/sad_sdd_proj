...
**使用方式**（在项目根目录 `c1_sdd` 下）::
    ...
    python format_docx_py/format_final_sdd.py
    python format_docx_py/format_final_sdd.py "D:\\path\\final_sdd.docx"
...
DEFAULT_TF = WORKSPACE_ROOT / "final_sdd.docx"
...
def _max_printable_box_emu(document: Document) -> tuple[int, int]:
    ...
    :param document: Document
    :return: ``(max_content_width_emu, max_content_height_emu)``
    """
    ...
    return (min_cx if min_cx is not None else 1), (min_cy if min_cy is not None else 1)
