...
MERMAID_DIR = MERMAID_SVGS
PANDOC_SRC = PANDOC_FOR_WORD
BODY_DOCX = BODY_GENERATED
...
def preprocess_mermaid(md_text: str) -> str:
    ...
    def repl(m: re.Match[str]) -> str:
        ...
        ctr["ok"] += 1
        rel = rel_to_workspace(svg).replace("\\", "/")
...
def rasterize_svg_refs_for_docx(md_text: str, *, clear_cache: bool = True) -> str:
    ...
    def repl(m: re.Match[str]) -> str:
        ...
        if not posix.lower().endswith(".svg"):
            return m.group(0)
        svg_path = (WORKSPACE_ROOT / rel_fs).resolve()
        if not svg_path.is_file():
            print(f"[warn] rasterize: missing {svg_path}", file=sys.stderr)
            return m.group(0)
        out_rel_fs = Path("cursor_tmp/_docx_raster") / rel_fs.with_suffix(".png")
        out_path = (WORKSPACE_ROOT / out_rel_fs).resolve()
