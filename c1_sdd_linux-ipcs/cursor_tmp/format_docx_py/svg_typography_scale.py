# -*- coding: utf-8 -*-
"""
Per-SVG display width from viewBox + font-size so Word renders ~5号 (10.5pt) body text.
...
# Calibrated against 3_3_1.svg: viewBox W=746, font-size 12, 14cm display ≈ readable 5号
REF_VIEWBOX_W = 746.0
REF_WIDTH_CM = 14.0
REF_FONT_PX = 12.0
TARGET_BODY_PT = 10.5  # 五号
...
def compute_typography_width_cm(viewbox_w: float, svg_font_px: float) -> float:
    if viewbox_w <= 0 or svg_font_px <= 0:
        return REF_WIDTH_CM
    return REF_WIDTH_CM * (viewbox_w / REF_VIEWBOX_W) * (REF_FONT_PX / svg_font_px)
