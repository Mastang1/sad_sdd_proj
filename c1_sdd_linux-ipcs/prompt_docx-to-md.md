...
# task-11: @md_sdd_0519.md 完善
 - 1. 总体任务，@md_sdd_0519.md文档中的章节5和6中，全局变量 和 类型定义 两个章节缺失。参考章节4.5、4.6的实现，补充章节5、章节6；注意：这两个章节中会有相同命名的结构体类型和全局变量，需要根据源码中的变体类型、实现中的OSAL添加必要描述；
 - 2. 要求：只可修改指定的操作，不要自行修改其他位置
 - 3. 要求：所有信息来自源码，必须与源码的结构体名称、变量名称等保持一致，不可虚构；
 - 4. 要求：表格格式参考rules及章节4.5、4.6