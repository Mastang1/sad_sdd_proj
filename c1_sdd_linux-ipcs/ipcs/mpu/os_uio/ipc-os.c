...
/* Rx sotfirq thread */
static void *ipcsShmSoftirq(void *arg)
{
	const int budget = IPC_SOFTIRQ_BUDGET;
	int irq_count;
	int work;
	struct IPCS_OS_PRIV_INSTANCE_TYPE *info = (struct IPCS_OS_PRIV_INSTANCE_TYPE *)arg;
		...
		int (*rx_cb)(const uint8_t, int))
{
	...
	struct IPCS_UIO_CDEV_DATA_TYPE data_cfg;
...
static void ipcsSendUioCmd(uint32_t uio_fd, int32_t cmd)
{
	int ret;
...
{
	ipcsSendUioCmd(ipc_os_priv.id[instance].uio_fd, IPC_UIO_ENABLE_CMD);
}
...
{
	ipcsSendUioCmd(ipc_os_priv.id[instance].uio_fd, IPC_UIO_DISABLE_CMD);
}
...
{
	ipcsSendUioCmd(ipc_os_priv.id[instance].uio_fd, IPC_UIO_TRIGGER_CMD);
}
